...
import HomeSlideKhac from './HomeSlideKhac'



const HomeKhac = (props) => {
    ...
    const getNvhInfo = async() => {
        try {
            ...
            const response = await fetch(`
                http://localhost:5000/getnvh/${id_nvh}`);
            const jsonData = await response.json()

            console.log('Homekhac:', jsonData);
            setNvhInfo(jsonData);
        } catch (error) {
            console.error(error.message);
        }
...
<div class="home-sl-bigdiv">
    ...
    */}
    <HomeSlideKhac id_nvh = {id_nvh} id_phong={id_phong}
    />
    <HomeGioiThieu
        info = {nvhInfo}
    />
