...
const Pheduyetdondk = (props) => {
  ...
  <div class="card pheduyet-card m-3 border border-secondary">
      ...
      <button class="Pheduyet-button-yes btn btn-success"
          type="button" data-toggle="collapse" data-target={`#collapse${dondk.id_dondk}`} aria-expanded="false" aria-controls={`collapse${dondk.id_dondk}`}
          onClick={()=>{
                setDondkinfo(prevValue => {
                return {
                    ...prevValue,
                ...
                setpheduyethaykhong("success")
                console.log('dondk:', dondkinfo);
              }}><BsFillCheckCircleFill/></button>
      <button class="Pheduyet-button-no btn btn-danger"
          type="button" data-toggle="collapse" data-target={`#collapse${dondk.id_dondk}f`} aria-expanded="false" aria-controls={`collapse${dondk.id_dondk}`}
          onClick={()=>{
                setDondkinfo(prevValue => {
                return {
                    ...prevValue,
                ...
                setpheduyethaykhong("danger")
                console.log('dondk:', dondkinfo);
              }}><BsFillXCircleFill/></button>
      
      <div class="input-group collapse pheduyet-input-mess" id={`collapse${dondk.id_dondk}`}>
      <input class="form-control" placeholder="Nhập lời nhắn của bạn cho khách hàng" name="hoidap" value={dondkinfo.hoidap} onChange={handleChange}>
          
      </input>
          <div class="input-group-append">
          <button class={`btn btn-${pheduyethaykhong}`} type="button" id="button-addon2" data-toggle="collapse" data-target={`#collapse${dondk.id_dondk}`}
                onClick={onEdit}
              >➲</button>
          </div>
      </div>
      <div class="input-group collapse pheduyet-input-mess" id={`collapse${dondk.id_dondk}f`}>
      <input class="form-control" placeholder="Nhập lời nhắn của bạn cho khách hàng" name="hoidap" value={dondkinfo.hoidap} onChange={handleChange}>
